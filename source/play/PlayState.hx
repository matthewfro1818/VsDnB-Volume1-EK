...
class PlayState extends MusicBeatState
{
	 ...
	 * When enabled, the bot will automatically hit all notes with perfect timing (Sicks).
	 */
	public var botplay:Bool = false;

	/**
	 * Tracks which note directions are currently being held for sustain notes during botplay.
	 */
	...
	/**
	 * The text display that shows when botplay is enabled (flashing "BOTPLAY").
	 */
	var botplayIndicator:FlxText;

	/**
	 * Timer for flashing effect of botplay indicator.
	 */
	private var botplayFlashTimer:Float = 0;

	/**
	 * The flash duration for the botplay indicator text.
	 */
	private final BOTPLAY_FLASH_DURATION:Float = 0.5;
	...
	override public function create():Void
	{
		...
		// Initialize botplay holding notes tracking
	...
	override public function update(elapsed:Float):Void
	{
		...
		// Update botplay indicator flashing effect
		if (botplayIndicator != null)
		{
			botplayFlashTimer += elapsed;
			if (botplayFlashTimer >= BOTPLAY_FLASH_DURATION * 2)
			{
				botplayFlashTimer = 0;
			}
			// Toggle visibility based on flash timer
			botplayIndicator.alpha = botplayFlashTimer < BOTPLAY_FLASH_DURATION ? 1.0 : 0.3;
		}
	...
	function initalizeUI():Void
	{
		...
		// Create botplay indicator if botplay is enabled
		if (botplay)
		{
			botplayIndicator = new FlxText(0, 0, 0, "BOTPLAY", 64);
			botplayIndicator.setFormat(Paths.font('comic.ttf'), 64, 0xFFFFFFFF, CENTER, OUTLINE);
			botplayIndicator.borderColor = 0xFF000000;
			botplayIndicator.borderSize = 4;
			botplayIndicator.screenCenter(X);
			// Position above health bar in upscroll, below in downscroll
			if (scrollType == 'downscroll')
				botplayIndicator.y = healthBar.y + healthBar.height + 10;
			else
				botplayIndicator.y = healthBar.y - botplayIndicator.height - 10;
			botplayIndicator.cameras = [camHUD];
			add(botplayIndicator);
		}
	...
	function createStrums():Void
	{
		...
		{
		    case 6, 7: dadStrums.x = 50; 
		    case 8, 9, 12: dadStrums.x = 0; 
		    default: dadStrums.x = 100;
		}
		...
		switch (Strumline.strumAmount)
		{
		    case 6, 7: playerStrums.x = FlxG.width - playerStrums.width - 50;
		    case 8, 9, 12: playerStrums.x = FlxG.width - playerStrums.width;
		    default: playerStrums.x = FlxG.width - playerStrums.width - 100;
		}
	...
	/**
	 * Handles botplay input simulation to automatically hit notes perfectly.
	...
	private function handleInputs():Void
	{
		...
		var second_upP = controls.SECOND_UP_P;
		var second_rightP = controls.SECOND_RIGHT_P;
		var second_downP = controls.SECOND_DOWN_P;
		var second_leftP = controls.SECOND_LEFT_P;

		var spaceR = controls.SPACE_R;
		var second_upR = controls.SECOND_UP_R;
		var second_rightR = controls.SECOND_RIGHT_R;
		var second_downR = controls.SECOND_DOWN_R;
		var second_leftR = controls.SECOND_LEFT_R;

		var third_upP = controls.THIRD_UP_P;
		var third_rightP = controls.THIRD_RIGHT_P;
		var third_downP = controls.THIRD_DOWN_P;
		var third_leftP = controls.THIRD_LEFT_P;

		var third_upR = controls.THIRD_UP_R;
		var third_rightR = controls.THIRD_RIGHT_R;
	...
	function badNoteCheck():Void
	{
		...
		switch(Strumline.strumAmount)
		{
			case 5:
		        controlArray = [leftP, downP, spaceP, upP, rightP];
			case 6:
		        controlArray = [leftP, upP, rightP, second_leftP, second_downP, second_rightP];
			case 7:
		        controlArray = [leftP, upP, rightP, spaceP, second_leftP, second_downP, second_rightP];
			case 8:
		        controlArray = [leftP, downP, upP, rightP, second_leftP, second_downP, second_upP, second_rightP];
			case 9:
		        controlArray = [leftP, downP, upP, rightP, spaceP, second_leftP, second_downP, second_upP, second_rightP];
