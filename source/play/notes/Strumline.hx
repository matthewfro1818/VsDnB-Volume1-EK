...
class Strumline extends FlxSpriteGroup
{
	...
	public function generateNotes(data:Array<SongSection>)
	{
		...
		for (section in chartData)
		{
			for (note in section.notes)
			{
				...
				if (note.direction > strumAmount - 1) gottaHitNote = !section.mustHitSection;
	...
	{
		if (FlxG.state is PlayState) strumAmount = PlayState.instance.currentChart.keyCount; else strumAmount = 4; // so that unintended things don't happen when this is used outside of playstate

		switch(strumAmount) 
		{
			case 5: NOTE_WIDTH = 160 * 0.6;
			case 6: NOTE_WIDTH = 160 * 0.525; 
			case 7: NOTE_WIDTH = 160 * 0.45; 
			case 8, 9: NOTE_WIDTH = 160 * 0.4;
			case 12: NOTE_WIDTH = 160 * 0.3;
            default: NOTE_WIDTH = 160 * 0.7;
		}
